...
- s.size() 返回的是 string::size_type 型別，這是一个無號（unsigned）型別的值，不要和 int 混用，事實上，大部分的程式庫型別，都定義有數個附帶型別，這些附帶型別能夠以獨立於機器的方式使用那些程式庫型別，而 size_type 就是其中的一種
...
- auto e = v.end(); 返回指向最後一個元素的下一個元素的迭代器（one past the end），這個迭代器又稱作尾端後迭代器或 end 迭代器（off-the-end iterator），代表該容器尾端後（off the end）一個不存在的元素
